#!/usr/bin/env python
#Code copied from https://raw.githubusercontent.com/VasuAgrawal/112-opencv-tutorial/master/TrackingFaces.py
...
class FaceDetector(object):
    ...
    def __init__(self):
        ...
        HISTORY = 60 #5 works well, 10 is laggy
    ...
    def findEyes(self, gray_face, face_pos):
        ...
        eyes = self.eyeCascade.detectMultiScale( #applies eyecascade
                gray_face_small,
        )
    ...
    def processFace(self, face_pos):
        ...
        cv2.rectangle(self.debug_frame, (x1, y1), (x2, y2), (1, 255, 1), 2) #originally self.debug_frame
        ...
        #ToDO
        #code I've added
        return (x1, y1, x2, y2) #gives coordinates of face
    ...
    def process(self, frame):
        ...
        print("faces:", faces)
        #list of lists
        for face_pos in faces: #let's assume length of faces is one... 
            # face_pos = (x, y, w, h)
            result = self.processFace(face_pos) #so we assume we detect one face
            print("result:", result)
            #print(frame)
            return result
        if faces == ():
